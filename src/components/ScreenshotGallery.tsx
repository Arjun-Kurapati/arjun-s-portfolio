      ...
      <div className="mt-6 flex flex-col items-center w-full">
        ...
        <div className="relative w-full max-w-4xl mx-auto flex items-center justify-center gap-4">
          ...
          <button
            ...
            className="flex-shrink-0 p-2.5 rounded-full bg-background/90 border border-border/50 text-foreground hover:bg-primary/20 hover:border-primary/50 transition-all duration-200 shadow-md"
          ...
          <div className="relative w-full max-w-3xl">
            <div 
              className="relative aspect-video bg-background-secondary rounded-xl overflow-hidden border border-border/50 cursor-pointer group shadow-lg"
        ...
        <div className="mt-4 flex gap-2 justify-center flex-wrap max-w-4xl">
          {images.map((image, index) => (
            <button
              ...
              className={`flex-shrink-0 w-16 h-10 md:w-20 md:h-12 rounded-md overflow-hidden border-2 transition-all duration-200 ${
                index === currentIndex
      ...
      <AnimatePresence>
        {isLightboxOpen && (
          ...
          >
            ...
            <div className="absolute top-4 right-4 flex items-center gap-2 z-10">
              ...
              <div className="flex items-center gap-1 bg-background/80 backdrop-blur-sm border border-border/50 rounded-full px-2 py-1">
                <button
                  ...
                  className="p-1.5 rounded-full text-foreground hover:bg-primary/20 transition-all duration-200 disabled:opacity-30 disabled:cursor-not-allowed"
                  aria-label="Zoom out"
                >
              ...
              <button
                onClick={closeLightbox}
                className="p-2 rounded-full bg-background/80 border border-border/50 text-foreground hover:bg-destructive/20 hover:border-destructive/50 transition-all duration-200"
            ...
            <motion.div
              className="max-w-[90vw] max-h-[85vh] relative overflow-auto"
            ...
            >
              <AnimatePresence mode="wait">
                <motion.img
                  key={`${currentIndex}-${zoomLevel}`}
                  src={images[currentIndex]}
                  alt={`Screenshot ${currentIndex + 1}`}
                  className="max-w-full max-h-[85vh] object-contain rounded-lg border border-border/30 shadow-2xl cursor-grab active:cursor-grabbing"
                  style={{ 
